...
#
#   sbatch -A <account> run_64A_benchmark.sh [max_iter]
#
...
cp -n "$REPO/benchmark/sim_64A_zenith.yaml" . 2>/dev/null || true
cp -n "$REPO/benchmark/tab_64A.yaml" . 2>/dev/null || true
...
if [ ! -f selected_norad_ids.txt ]; then
    $PIXI run --manifest-path "$REPO/pyproject.toml" -e "$ENVNAME" python3 \
        ...
        --sim-config sim_64A_zenith.yaml --names starlink -n 4 \
...
$PIXI run --manifest-path "$REPO/pyproject.toml" -e "$ENVNAME" python3 \
    ...
    --sim-config sim_64A_zenith.yaml --ids selected_norad_ids.txt \
    --target-snr 200 --write-back
...
if [ ! -f sim_dir.txt ]; then
    $PIXI run --manifest-path "$REPO/pyproject.toml" -e "$ENVNAME" python3 \
        ...
        > .simvis_path
    $PIXI run --manifest-path "$REPO/pyproject.toml" -e "$ENVNAME" python3 \
        "$(cat .simvis_path)" -c sim_64A_zenith.yaml
