...
The same two priors, written on the data grid rather than the fine integration grid: one value per channel and time step, at the centre of the cell it stands for. Latent, spectrum, parameters and initialisation are their parents' -- the transform simply leaves out the supersampling, so where the two grids overlap they agree exactly. They exist for {class}`~tabascal.components.rfi_vis.GPInterpVis` and {class}`~tabascal.components.rfi_vis.PolyInterpVis`, which form the fine samples the visibility integral needs from this grid -- under the same Gaussian process, or off a polynomial through the neighbouring values; each leaves the spectrum it samples on the configuration for those components to read. The Riemann-sum kernels read the fine grid and refuse a data-grid `rfi_A` by shape.
