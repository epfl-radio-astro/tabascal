...
A run with [`data.save_rfi_per_sat: true`](config.md) stores one more variable, `rfi_vis_src (sample, src, bl, freq, time)`, with a `norad_id` coordinate on the `src` axis: the same `rfi_vis` split into its per-satellite contributions. It is `n_rfi` times the size of `rfi_vis` on disk, which is why it is opt-in — but only one `(sample, satellite)` block of it is in memory at a time, since each is written into the store as it is evaluated.
