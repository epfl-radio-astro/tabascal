...
An unflagged dead antenna can be driven to a zero or non-finite gain by the fit. Any such antenna gain is replaced by 1 before anything is derived from it, and a `RuntimeWarning` is raised naming the affected antennas and the number of gain samples substituted. Baselines touching such an antenna are then written *uncalibrated on that antenna* — the other antenna's gain is still applied — rather than being blanked. Nothing is ever written as `NaN` or `inf`, so every column stays finite and imageable. The `vis_obs` stored in the results `.zarr` predates the substitution, so on those baselines `TAB_RES_DATA` falls back to re-deriving the total as `g_p g_q* × ast + g_p g_q* × rfi` from the substituted gains.
