...
* `gp_cov`: The prior covariance of the astronomical visibilities: a width, the correlation scales that set the knee, and the roll-off past it. It is diagonal in the Fourier domain, which is how it is applied, and its diagonal is a power spectral density — but what you are setting here is the covariance of a Gaussian process, not the power spectrum an observation is trying to measure.
  ...
  [`rfi.gp_cov`](#rfi-signal) takes the same keys at the same level, and the same validator checks both: each of `std`, `corr_freq`, `corr_time` and `fov_deg` must be a finite positive number (or one of the words its own entry names), and `gammas` an ordered pair of them — one for the frequency axis and one for the time axis, in that order. A key the section does not read is refused by name rather than ignored, so a `gamma` written for `gammas` is caught rather than silently doing nothing.
...
* `std`: The width of the prior on the astronomical visibilities, in Jy — the standard deviation of the *complex* visibility about its prior mean, $\sqrt{E|V-\mu|^2}$. At the default `mean: 0` that is exactly `rms|V|`. Under `mean: data` the prior is centred on the observed visibilities and `std` is the scatter allowed around them rather than the total amplitude, which is then $\sqrt{\mathtt{std}^2 + |\mu|^2}$. The power spectrum is normalised to it, so this *is* the prior width: not its square, not its square root, and not a constant times it.
  ...
  **`data`, the default**, measures it for you: `rms|V|` per baseline over the samples nothing has flagged. At the default `mean: 0` that is exactly what `std` is defined as, so there is no conversion in between (under `mean: data` the two differ, since the prior is then centred on the visibilities rather than on zero), and it gives one width per baseline rather than one for all of them.

  **It measures whatever is in the unflagged samples, RFI included.** Where the contamination has been flagged, the estimate is the sky. Where it has not, the estimate is the contamination: on the shipped 8A simulation, whose RFI is unflagged because modelling it is the job, `data` returns about 11 Jy against a true sky under 3. It prints what it measured and warns when nothing -- neither the MS nor the gain table -- excludes a single sample. Note that it reads the MS's flags directly, **whatever `data.flags` is set to**, along with the samples no gain table could calibrate — those answer different questions. `data.flags` decides whether tabascal declines to *fit* a flagged sample; this decides where it is willing to *measure a clean sky* from. A strong emitter flagged by some other task is exactly the data tabascal is here to recover, so you want it in the likelihood and out of the amplitude estimate, and that is what you get. An uncalibratable sample is out of both: it kept a unity gain where its neighbours were divided by a real one, so it is not on the same flux scale as the data around it. [#220](https://github.com/epfl-radio-astro/tabascal/issues/220) tracks estimating it from the RFI model's own view of which samples are clean, which is what would make the default estimate the sky rather than the sky plus whatever sits on top of it.

  **To set it by hand**, take the visibility amplitude in a channel with no visible RFI. The shipped configs are set from the true sky of the simulation each runs on, at half the widest baseline's amplitude, so every baseline's sky sits inside 2 σ of the prior. Covering the widest at 1 σ instead was measurably worse on both counts: on the FixedOrbit case it cost 17 % of the astronomical accuracy (NRMSE 0.2089 against 0.1784) and raised the coherent bias from 1.2 σ to 1.8 σ. Where no such channel exists, the amplitude of the shortest baselines is the usual stand-in, since they see the most sky flux and the least resolved structure. It is a prior width, not a measurement, so being within a factor of a few is enough — what matters is that it is the right order of magnitude and that it means the same thing from one config to the next.

  **Why it is a width and not a power.** `cutoff` exists to drop modes that carry no power, and `gammas` and `fov_deg` shape the roll-off; none of them is a statement about how bright the sky is, so none of them moves this number. The spectrum is normalised to `std` after the cut, which is what makes that true.

  Every baseline gets this width. The time-axis knee is that baseline's own maximum fringe rate, so each baseline's spectrum is normalised separately; unnormalised, the shipped configuration spans a factor of 1.23 across its baselines.
...
* `gp_cov.std`: The width of the prior on the RFI signal, in Jy: the RFI's typical `rms|V|`. **The same quantity as [`ast.gp_cov.std`](#astronomical-signal)** — read an amplitude off the data and write it here, for either prior.
  ...
  * **The MS's flags are kept.** The astronomical estimate excludes them because whatever a flag means, the sample is not clean sky. The reverse does not follow: a flag says *something is wrong here*, not *RFI is here*, and a dead antenna is neither RFI nor a scale to set an RFI prior from. Measuring the flagged samples is also biased high even when they are RFI, wherever the flagger thresholds on amplitude: the flagged half is then the bright half — on the shipped 8A simulation that returns 1.54x the true RFI where measuring everything returns 1.015x. Samples no gain table could calibrate *are* dropped, which is the one exclusion both priors share.
  ...
  `matched-filter` (or `mf`) measures it **per satellite**, from the same matched-filter light curves `init` and `mean` can seed from (filtered once for all three). Each source gets the debiased rms of its own curve over the samples it is in view, $\texttt{std}_i^2 = \max(\langle\lvert\hat S_i\rvert^2\rangle - 2\langle\epsilon_i^2\rangle,\ \langle\epsilon_i^2\rangle)$ with $\epsilon$ the filter's error, so a satellite it did not detect takes the filter's own floor rather than zero. It is the only measurement here that is one source's: `data` and `null` hand every satellite a number taken from *all* the visibilities, which on an array with short baselines is the diffuse sky's — $7.7\times10^5$ Jy on one low-frequency observation whose satellites are $10^2$–$10^4$ Jy. A satellite then sits at a few hundredths of its width in the whitened parameters, and because the visibility is quadratic in the amplitude, zero is a stationary point a fixed-step optimiser can walk a faint satellite into and leave it: reduced $\chi^2$ unchanged, the satellite unsubtracted. The sky under a track is in the filter's estimate too, so the width tends wide, which is the safe side. Requires a trajectory component, as `init: matched-filter` does.
...
* `mf_sky`: What the matched filter takes off the visibilities before it filters them, for every key that reads its light curves — `init`, `mean`, `gp_cov.std`, `gp_cov.corr_freq`, `gp_cov.corr_time` — and for `tabascal light-curve -c`. `null`, the default, filters them as given. `median` first subtracts, per baseline and channel, the median over time of the real and of the imaginary part (flagged samples left out).

  The filter beam-forms whatever the visibilities hold toward the track, and on an array with short baselines that is mostly the sky. On a 151 MHz pass of one such array it reads a track S/N of 200 for three satellites whose own signal is 20–35; the sky's share is slow and of either sign, so the curve is negative for a third of the pass, the seed $\sqrt{\max(\mathrm{Re}\,\hat S, 0)}$ is zero there — and zero is a stationary point of a model quadratic in the amplitude, so the fit can leave those stretches unsubtracted with the reduced $\chi^2$ unchanged — and the width and scales measured from the curve are the sky's (335–495 Jy; 85–215 Jy with the median off, against 95–218 Jy with the run's *fitted* sky off). Over a pass the sky is nearly static on a baseline, while a satellite in low orbit turns the fringe of even the shortest baseline at least once, so the time median is the sky and not the satellite, and it needs no model.

  What it costs: an emitter whose fringe does not complete a turn on a baseline within the pass — slow or distant, a very short baseline, a pass of a few integrations — loses part of its flux on that baseline to the median. The estimate seeds and scales a fit, which forgives a few per cent. Fewer than three integrations have no median to take and are filtered as given.
...
* `gp_cov`: The prior covariance of the RFI signal, taking the same keys as [`ast.gp_cov`](#astronomical-signal) at the same level and checked by the same validator: `std`, `corr_freq`, `corr_time` and `gammas`. Every one of them is optional, and each `null` has its own meaning, given below.
  ...
  * `corr_time`: the time, in seconds, over which the RFI signal stays correlated, setting the knee along the time axis. `null` is half the observation. It is given as a time where the astronomical block asks for `fov_deg`, because an emitter's coherence time is a property of the emitter, while a sky source's is its fringe rate — a consequence of how far off axis it sits rather than a number to pick.
  ...
  Both scales also take `matched-filter` (or `mf`), and are then measured **per satellite** from the light curves `std`, `mean` and `init` share. The de-rotated RFI is real, so $\mathrm{Re}\,\hat S_i$ is the light curve and $\mathrm{Im}\,\hat S_i$ a matched null — the same noise and confusion, no signal; antenna phase errors shrink the real part without moving the signal out of it. Two scales are read off each axis and the shorter is used:

  * the **1/e lag** of the autocorrelation of $\mathrm{Re}\,\hat S_i$ with its white-noise variance taken off the zero lag — in time of the inverse-variance channel average over the longest stretch in view, a straight line removed (the sky under a track is slow power the satellite does not have, and reads 5–30 s long on the low-frequency passes this was written against); in frequency of the channel series against separation. A correlation that never reaches $1/e$ in the lags there are is extrapolated from the decay it did show, not called the extent: emission at $\rho = 0.95$ across a 0.75 MHz band has a 15 MHz scale, and calling it the band hands the prior seventeen frequency modes for a signal that has one. Capped at 64 extents.
  * the **envelope scale**: the longest scale whose prior spectrum, normalised to the amplitude's power, is nowhere below the power spectrum of $\sqrt{\mathrm{Re}\,\hat S_i}$ in any bin detected above the null's. An e-folding time is the scale of the dominant variation; a bright source also has fast structure that carries almost none of the variance and all of the $\chi^2$ — SOLRAD 7B's carrier e-folds in 28 s and is fitted at $\chi^2 = 36$ with that, where 5 s gives 0.95. The floor is the null's spectrum bin by bin, not the filter's analytic error, which assumes independent baselines and sat 3–15× below $\mathrm{Im}\,\hat S$ where this was measured.

  A satellite the filter did not detect takes the median of those measured; an axis nothing measures — one channel, a handful of integrations — takes what `null` means there. One pass is one realisation, so a scale is good to a few tens of per cent, which is the precision of a knee.

  The latent grid is shared, so it is cut for the shortest scale present and every satellite keeps its own spectrum on it: `sigma_rfi_k` is then `(n_rfi, 1, n_k_freq, n_k_time)` with a shape per source, not only a scale. A mode kept for a faster satellite is held at the cutoff's level for a slower one, axis by axis, which is the rule the grid was cut by. The cost is the grid: the fastest satellite sets the number of fitted parameters for all of them.

  Measured on six subsets of one low-frequency observation: 15–25 s for five 151 MHz Starlinks whose config said 180 s, 9–22 s for five at 170 MHz (80 s), 28 s for SOLRAD 7B where a hand-tuned 24 s already worked; every Starlink coherent across the 0.75 MHz there was, SOLRAD's channels 6–17 at 43 kHz. The long values were invisible while `std` was $10^3$ times the signal and the roll-off never bit. One thing a single scale does not carry: a bright satellite has significant structure well inside its e-folding time, and `gammas` decides whether the prior leaves room for it.
