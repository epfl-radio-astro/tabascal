...
* `poly_analytic`: Read by `rfi_vis:PolyInterpVisHybrid`. Baselines whose rounded requirements exceed `quadrature_limit` use an analytic time integral; the rest retain quadrature. `quadrature_limit: null` derives a conservative crossover from the arithmetic work, giving **1376 samples** for the default quadratic amplitude stencil, `segments: 4`, `terms: 16` and `cubic_terms: 3`. Thus odd counts through 1375 use quadrature and 1377 upwards use the analytic integral. An explicit non-negative count overrides the cut; `0` sends every baseline through the analytic route. `segments` splits the analytic integral into equal pieces, reducing local curvature by its square; `terms` sets the curvature series length. `cubic_terms` sets the residual cubic-phase expansion length (default `3`); `0` drops it for comparison. Segments and curvature terms must be positive whole numbers, and cubic terms non-negative. The defaults are tested through 1000 noninteger winding turns and 3 curvature turns at the cell edge; larger curvature needs more segments. The analytic route integrates the full amplitude polynomial against quadratic phase and a perturbative cubic correction; delay derivatives above order three are omitted. Dropping cubic phase costs less than 0.7% of the noise on the supplied median/p99 combinations, but 14–72% in slowly winding cells with the p99 cubic coefficient. Group membership uses a maximum over cells and sources, so the correction stays enabled by default. These are tests of the supplied coefficient range, not a bound for arbitrary geometry. Frequency integration and its offsets are unchanged. The older `poly_time_sampling` options are read only by the two quadrature-only variable components.
