...
* `baseline_block_size`: The number of baselines `RiemannVis` calculates per step of its scan over the baseline axis, a whole number defaulting to `128`. The Riemann sum is formed on a `(n_bl, n_rfi, n_freq_fine, n_time_fine)` fine grid before anything is reduced, which is `n_rfi * n_int_freq * n_int_time` times the size of the visibilities it reduces to and, under reverse-mode automatic differentiation, is what the tape holds; the scan replaces `n_bl` in that shape with `baseline_block_size`, or with `n_bl` itself where that is smaller, and recomputes each block in the backward pass rather than keeping it. It is purely a memory strategy — baselines are independent, so the result does not depend on it — at the cost of recomputation and of one scan step per block. `null` is the setting for a single block over every baseline: the checkpoint stays, so the backward pass still recomputes the fine grid instead of storing it, but the grid is formed whole. That bounds the tape and not the peak, and it is the setting that trades the memory back for the scan's step overhead. `RiemannVis` and `GPInterpVis` read it: the FFI kernels bound the same term inside the compiled kernel, and the `Variable` components carry their own baseline grouping. See [RFI-visibility kernels](kernels.md#memory).
* `gp_interp_stencil`: The half-width, in data-grid cells, of the block of coarse values `rfi_vis:GPInterpVis` interpolates each cell's fine samples from, a whole number defaulting to `1`: the 3 x 3 block around the cell, nine values and a 9 x 9 prior covariance; `2` is the 5 x 5 block. More neighbours interpolate closer to the supersampled grid, at `(2h + 1)^2` multiplies per fine sample and a worse-conditioned solve -- made once, at setup, in float64. An axis with a single integration sample is left out of the block whatever this is, since its one fine sample is the coarse value itself. Read by no other component; see [RFI visibility components](components/rfi_vis.md).
* `poly_interp_degree`: The degree of the polynomial `rfi_vis:PolyInterpVis` reads each cell's fine samples off, on each axis, or `null` -- the default -- for the polynomial through the stencil values alone: degree `2 x gp_interp_stencil`, the quadratic through a cell and its two neighbours at the default stencil, which needs no solve and nothing from the prior. A whole number of at least that raises the degree; the polynomial still passes through the stencil values, and the extra coefficients are the prior's expectation of them given those values, from the spectral moments of the spectrum the signal component samples, so the weights converge to `rfi_vis:GPInterpVis`'s as the degree grows. Measured at the default stencil `3` reproduces that component's accuracy and `4` its weights; at `gp_interp_stencil: 2` it takes `8`, and lower values are no better than `null`. Read by no other component; see [RFI visibility components](components/rfi_vis.md).
* `time_block_size`: The number of time steps `rfi_vis:GPInterpVis` interpolates and integrates per step of a scan over the time axis, a whole number, or `null` -- the default -- for every time step at once and no scan. It bounds the fine grid that component holds to the block's worth of time steps, the way `baseline_block_size` bounds it along the baseline axis, and the two compose; each block is recomputed in the backward pass rather than kept, as the baseline blocks are. Like the baseline block it changes the value of nothing. Read by no other component.
* `path_order`: The order of the time expansion of the RFI geometric path within an integration that `trajectory:FixedOrbitCoarse` and `trajectory:PathCalculationRFI` carry beside the data-grid phase, for `rfi_vis:GPInterpVis` to rebuild the fine-grid phase from, a whole number defaulting to `3`: the path, its rate, acceleration and jerk at each time step. Order 0 holds the path at its centre value, which is only right where no fringe turns within an integration. The frequency axis needs no order -- the phase is linear in frequency and that term is exact. On a low-Earth-orbit pass the rebuilt phase agrees with the fine grid's to a hundredth of a degree from order 2 up; see [Trajectory components](components/trajectory.md). Read by no other component.
