...
satellites:
  ...
  remote_tle_max_age_days: 3
  tle_catalogue_interval_hours: 2
  tle_catalogue_settle_days: 45
  tle_provisional_cache_hours: 12
  ric_std: 1e2
```

* `norad_ids`: List of the NORAD IDs of the satellites to include. Their orbital elements (TLEs) are fetched automatically from the [IAU CPS SatChecker](https://satchecker.cps.iau.org/) service — no account or credentials are required — and cached locally for reuse. If the service reports no bulk catalogue at the observation epoch (its catalogue has a data horizon, so very recent observations can fall beyond it), each satellite is fetched individually instead. **Every ID listed here must resolve to an acceptable TLE**: if any does not, the run stops during preflight — before the visibilities are read and long before subtraction — with an error naming each failing satellite, how close the best available record was, and the remedies. TABASCAL never silently drops a configured satellite from the RFI model. See [Two-Line Elements (TLEs)](tles.md) for the full caching behaviour, how to reproduce a run's TLEs exactly, and how to supply TLEs manually when the service cannot.
...
* `remote_tle_target_age_days`: Age, in days, above which a record taken from the **bulk** catalogue is re-requested from the per-satellite endpoint (default `1`; `null` disables the upgrade pass). This exists because of the endpoint asymmetry described in [Which endpoint answered changes the TLE age](#sec-endpoint-freshness): `tles-at-epoch` returns the newest record at or *before* the requested epoch, so it cannot return a closer record that falls just after it, while `get-nearest-tle` can. Records already obtained from the per-satellite endpoint are never re-requested — they are already the nearest the service holds.

  The cost is one request per upgraded satellite; the benefit, measured over the 32 GNSS satellites of the bundled benchmark, was a worst-case age of 1.06 d instead of 4.53 d for 11 requests rather than 32. Lower values improve the median but not the worst case. Must not exceed `remote_tle_max_age_days`, since a record staler than the ceiling is rejected rather than upgraded; configuring it higher is a configuration error. An upgrade can only ever *improve* a record: if the endpoint has nothing fresher, cannot answer, or offers something beyond the ceiling, the existing record is kept and the run is unaffected.
* `tle_catalogue_interval_hours`: Width, in hours, of the fixed UTC bucket used to reuse a managed catalogue snapshot (default `2`; minimum one second, or `1/3600` hour). Buckets are globally anchored at the Unix epoch and a request is served by the SatChecker catalogue nearest each bucket's midpoint, so the reused snapshot depends only on the request and this width — never on what is already cached. This is a deliberate approximation: a returned record is nearest to the canonical bucket epoch, *not* necessarily nearest to the exact observation, and two observations less than one bucket-width apart can still straddle a boundary and use different snapshots. (The age check above is made against the exact observation epoch, not the bucket.)

  Note that with the default two-hour bucket this contributes at most one hour of extra TLE age, and **it is not the dominant term** — which endpoint answered matters far more. See [Which endpoint answered changes the TLE age](#sec-endpoint-freshness).
* `tle_catalogue_settle_days`: How old an observation epoch must be (default `45` days) before its SatChecker catalogue is treated as settled and stored as an immutable snapshot. A newer or future epoch is cached only *provisionally*, under its own filename, and is never promoted — once the epoch settles, the next run downloads a fresh stable snapshot. This exists because SatChecker's full-catalogue ingest was observed ramping from 9 rows at an observation age of 16 days to 31,108 at 30 days, so a response can be complete against its own `total_results` while the upstream catalogue is still filling. **45 days is an observed defensive caching policy, not a SatChecker API guarantee, and not a statement that a 45-day-old TLE is usable** — that is `remote_tle_max_age_days`' separate job. `null` is an expert opt-out treating every catalogue as settled.
* `tle_provisional_cache_hours`: Lifetime, in hours, of a provisional catalogue snapshot (default `12`). It exists purely to avoid repeating a multi-megabyte download within one working session; after it expires the catalogue is fetched again.
