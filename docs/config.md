...
* `std`: The width of the prior on the astronomical visibilities, in Jy — the standard deviation of the *complex* visibility about its prior mean, $\sqrt{E|V-\mu|^2}$. At the default `mean: 0` that is exactly `rms|V|`. Under `mean: data` the prior is centred on the observed visibilities and `std` is the scatter allowed around them rather than the total amplitude, which is then $\sqrt{\mathtt{std}^2 + |\mu|^2}$. The power spectrum is normalised to it, so this *is* the prior width: not its square, not its square root, and not a constant times it.
  ...
  **`data`, the default**, measures it for you: `rms|V|` per baseline over the samples nothing has flagged. That is the same quantity `std` is defined as, so there is no conversion in between, and it gives one width per baseline rather than one for all of them.

  **It measures whatever is in the unflagged samples, RFI included.** Where the contamination has been flagged, the estimate is the sky. Where it has not, the estimate is the contamination: on the shipped 8A simulation, whose RFI is unflagged because modelling it is the job, `data` returns about 11 Jy against a true sky under 3. It prints what it measured and warns when nothing is flagged; [#220](https://github.com/epfl-radio-astro/tabascal/issues/220) tracks estimating it from the RFI model's own view of which samples are clean.

  **To set it by hand**, take the visibility amplitude in a channel with no visible RFI. Give the widest baseline's amplitude rather than the typical one, so that every baseline's sky sits inside 1 σ of the prior — that is how the shipped configs are set, from the true sky of the simulation each runs on. Where no such channel exists, the amplitude of the shortest baselines is the usual stand-in, since they see the most sky flux and the least resolved structure. It is a prior width, not a measurement, so being within a factor of a few is enough — what matters is that it is the right order of magnitude and that it means the same thing from one config to the next.

  **Why it replaces `p0`.** `p0` was the power at $k=0$ of a spectrum that nothing normalised, so the width it produced was $\sqrt{p_0}\,\sqrt{2\sum\mathrm{shape}/N}$ — $\sqrt{p_0}$ times a factor that depended on `gammas`, `cutoff`, `fov_deg`, `corr_freq` and the padded grid, differed from baseline to baseline, and was written down nowhere: on the shipped 8A configuration the old `p0: 3e3` came to a prior 26 Jy wide (25.8 to 31.7 across its baselines), so `p0` was giving the shipped configuration a prior an order of magnitude wider than its own sky — the true sky there is 0.27–2.90 Jy per baseline, which is where the shipped `std: 2.9` now comes from. Worse, `cutoff` exists to drop modes that carry no power — an efficiency setting — yet raising it from 1e-6 to 1e-3 widened that same prior by 34 %, and shallower `gammas` narrowed it by 22 %. There is no conversion from one to the other, because it is not a single number; a config carrying `p0` is refused with that guidance rather than reinterpreted.
