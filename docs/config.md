...
* `skip_ms_write`: Leave the run's results in the zarr and skip copying them into the Measurement Set. Default `false`. A fit writes its products twice — once into `results/`, and again into the MS's data columns — and on a large observation the second copy is a substantial part of a run that is otherwise finished. With this set the run stops after the zarr, and `tab2MS` copies the same products across later, so nothing is lost and the choice is deferred rather than made in advance. Useful when the MS is on slow or shared storage, when several fits are being compared and only one of them will be kept, or when the MS is read-only. Also available as the `--skip-ms-write` flag, which turns it on whatever the config says; the flag only enables, so a config with it set true is not overridden by leaving the flag off.
* `row_chunk`: The number of MS rows read and written per chunk, or `null` (the default) to leave dask-ms's own chunking alone. It is a throughput and memory knob with no effect on results: a larger chunk means fewer, bigger reads, which is usually faster until the chunk stops fitting comfortably in host memory. The right value depends on the machine and the file rather than on the observation, which is why there is no default worth shipping — leave it unset unless MS I/O is measurably costing you. Must be a positive whole number if given.
...
* `poly_interp_stencil`: Read by the `PolyInterpVis` family, the data-grid route (see [The data-grid RFI route and its kernel boundary](coarse_rfi_vis.md)). The number of data cells on either side of a cell that its interpolant runs through, on each axis, so that `2 * poly_interp_stencil + 1` coarse samples make each cell's fine samples: `1`, the default, is the quadratic through a cell and its two neighbours; `0` holds each cell's value across it; `2` is the quartic through five. An axis with fewer cells than the stencil needs -- a single channel, say -- takes the widest stencil it can hold, which for one cell is its own value. A whole number of at least 0.
* `poly_time_sampling`: Read by `PolyInterpVisVariable` and `PolyInterpVisVariableFFI`. `max_groups` is `1` or `2` (default `2`); `split_at` is a positive whole sampling count or `null` (default). The polynomial routes round each fringe-rate requirement up to a positive odd count: the even-count `fine_offsets` grid is shifted half a sample left, and its phase error is not controlled by the midpoint-quadrature estimate above. The non-variable polynomial route uses the maximum of these counts, with no legacy divisor rounding. Each variable group builds independent interpolation tables at its own count and compacts its antenna inputs, so one group reproduces that non-variable route. With two groups allowed, setup searches every threshold between distinct requirements and minimises `sum(n_g * number_of_antennas_in_group)`. Shared antennas count in both groups. This score is materialisation work, not predicted runtime. `split_at` restricts the search to requirements at or below that count versus those above it; empty splits and splits that do not strictly beat one group fall back to one group. Equal scores favour fewer groups, then the lowest threshold. The legacy stride keys do not affect polynomial sampling; with no satellites its count is `1`.
* `poly_analytic`: Read by `rfi_vis:PolyInterpVisHybrid` and `rfi_vis:PolyInterpVisHybridFFI`. Baselines whose rounded requirements exceed `quadrature_limit` use an analytic time integral; the rest retain quadrature. `quadrature_limit: null` uses the measured GH200 VJP crossover: **166 samples in single precision, 56 in double**, because the optimiser runs the transpose every iteration. Re-measure on very different hardware; quadrature's memory traffic makes operation counts a poor predictor. An explicit non-negative count overrides the cut; `0` sends every baseline through the analytic route. Empty groups create no operator. `segments` splits the analytic integral into equal pieces, reducing local curvature by its square; `terms` sets the curvature series length. Defaults are `segments: 2`, `terms: 6`, `cubic_terms: 3`: measured accuracy was 1.21e-5 at a fifth of the 4/16 cost. This measurement is not a bound for arbitrary geometry. The broader stress tests through 3 curvature turns at the cell edge use an explicit 4/16 expansion; increase segments or terms for larger curvature or tighter accuracy. `cubic_terms` sets the residual cubic-phase expansion length; `0` drops it for comparison. Segments and curvature terms must be positive whole numbers, and cubic terms non-negative. The analytic route integrates the full amplitude polynomial against quadratic phase and a perturbative cubic correction; delay derivatives above order three are omitted. Frequency integration and its offsets are unchanged. The FFI route requires `ri_kernels` from `interp-analytic` and differentiates amplitudes through both groups; analytic phase and delay inputs are fixed. The older `poly_time_sampling` options are read only by the two quadrature-only variable components.
* `path_order`: Read by `trajectory:FixedOrbitCoarse` alone. The degree of the polynomial in time fitted through each cell's fine-grid path, whose coefficients -- the path and its first `path_order` time derivatives at the cell centre -- `rfi_vis:PolyInterpVis` rebuilds the fine phase from. `3`, the default, keeps the phase difference a baseline sees to a few thousandths of a degree over an 8 s integration on the test geometry; `1` is a few tenths of a degree there and worse on longer baselines. A cell with fewer fine samples than the degree needs is fitted to the degree it can hold. A whole number of at least 0.
...
Baseline sharding pads each device’s kernel work to the largest local group count and adds enough dark antennas for distinct ghost pairs. Ghost rows are discarded. If the total baseline count is indivisible by the device count, the internal block size rounds up; public arrays retain their original shape and use replicated placement at component boundaries. Divisible totals keep baseline-sharded parameters, constants, visibilities, and optimizer buffers.
