...
* `truth_zarr`: The tab-sim simulation truth, read only by `ast.init: truth` and `plots.truth`. Defaults to `<MS directory>/<MS name>.zarr` — beside the visibilities, which is where `sim-vis` writes the two together — so a simulation needs it only when its truth has been moved away from its MS. A real observation has no truth, and a path that does not exist is reported as "No tab-sim truth available" rather than failing the run.
