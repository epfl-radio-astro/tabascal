...
satellites:
  ...
  tle_cache_reuse_max_age_days: 1
  ric_std: 1e2
```
...
* `extra_tle_max_age_days`: Maximum allowed absolute difference, in days, between an `extra_tle_dir` TLE's epoch and the observation epoch. `null` (default) applies no age limit, preserving exact replay of `used_tles_*.json`; `0` accepts an epoch match within TLE precision. A rejected local record falls through to the managed cache and SatChecker. The age is parsed from line 1, not the filename or modification time.
* `remote_tle_max_age_days`: Hard ceiling, in days, on how far a SatChecker or managed-cache TLE may be from the observation. The epoch is parsed locally from line 1, and every accepted remote record's source, provider, signed offset and absolute age is logged. `null` explicitly removes the ceiling.
...
* `tle_cache_reuse_max_age_days`: Request-avoidance threshold for the per-NORAD cache (default `1`). A cached TLE this close to the observation avoids a request. An older cached record triggers an exact-epoch nearest lookup but remains an offline fallback if it is within `remote_tle_max_age_days`. `null` always reuses the nearest acceptable cached record. When both limits are set, this value must not exceed the hard ceiling.
