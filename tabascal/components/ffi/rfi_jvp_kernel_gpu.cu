...
__global__ void __launch_bounds__(BLOCK_SIZE)
    rfi_jvp_kernel(double scale, Tensor1D<const int *, INT_T> a1,
    ...
    for (INT_T i_f = blockIdx.z; i_f < n_freq; i_f += gridDim.z) {
      ...
      for (INT_T i_t = blockIdx.x; i_t < n_time; i_t += gridDim.x) {
        ...
        for (INT_T i_red = threadIdx.x; i_red < n_reduce; i_red += BLOCK_SIZE) {
          ...
          g1 = cuCadd(g1, cuCmul(val_rfi_amp_1, cuConj(val_rfi_amp_grad_2)));
          ...
          auto g2 =
              cuDoubleComplex{val_rfi_phase_grad_1 - val_rfi_phase_grad_2, 0};
...
ffi::Error calc_rfi_jvp_gpu_dispatch(
  ...
  if (rfi_phase_tensor.shape[3] / 2 < 32) {
    ...
    rfi_jvp_kernel<block_size, INT_T><<<grid, block, 0, stream>>>(
