...
Complete reference for all columns returned by Space-Track.org GP/GP_History API
when retrieving satellite orbital data in JSON/OMM format.
...
EPOCH (datetime, nullable)
    Reference time for the orbital elements (UTC).
...
INCLINATION (float, nullable)
    Orbital inclination in degrees (0° to 180°).
...
RA_OF_ASC_NODE (float, nullable)
    Right Ascension of Ascending Node (RAAN) in degrees (0° to 360°).
...
ARG_OF_PERICENTER (float, nullable)
...
CLASSIFICATION_TYPE (str, nullable)
...
MEAN_MOTION_DOT (float, nullable)
...
MEAN_MOTION_DDOT (float, nullable)
...
These fields are computed from the Keplerian elements for convenience.
Space-Track added these to allow filtering queries without client-side computation.

SEMIMAJOR_AXIS (float, nullable)
...
RCS_SIZE (str, nullable)
    ...
    - Helps estimate object dimensions (rough):
...
TLE_LINE1 (str, nullable)
    Line 1 of the TLE in fixed-width text format.
    - Contains: catalog ID, classification, international designator,
      epoch, mean motion derivatives, BSTAR, ephemeris type, element set number
...
TLE_LINE2 (str, nullable)
    Line 2 of the TLE in fixed-width text format.
    - Contains: catalog ID, inclination, RAAN, eccentricity,
      arg of perigee, mean anomaly, mean motion, revolution number
    - 69 characters, ends with checksum digit (modulo-10)
    - Example:
...
NO COVARIANCE DATA:
    Space-Track does NOT provide uncertainty/covariance matrices in
    standard GP data. For uncertainty estimates, you must:
...
DERIVED FIELDS:
    SEMIMAJOR_AXIS, PERIOD, APOAPSIS, PERIAPSIS are computed from
    the Keplerian elements and may not exactly match SATCAT values
...
- CCSDS 502.0-B-3: Orbit Data Messages (OMM Standard)
